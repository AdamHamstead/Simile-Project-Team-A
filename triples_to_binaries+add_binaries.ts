...
export function triples_to_binaries(rfname:string){
    ...
    for(let i:number = 0; i<gb.numReps; i++){
        ...
        console.log("\n\n" + times + " direct pathways from \"" + gb.concepts[source] + " - " + relation_labels[relation] + " - " + gb.concepts[target] + "\" to \"" + gb.concepts[output] + "\"");
    ...
    if(gb.context[target][attribute] == true){
        ...
        if(f.is_output(target) && f.is_input(attribute)){
            ...
            for (let p = 0; p < pathSize; p++){
                console.log(gb.concepts[path[p][0]] + " - " + relation_labels[path[p][1]] + " - ");
                fs.writeFileSync(rfname, gb.concepts[path[p][0]] + " - " + relation_labels[path[p][1]] + " - ");
            }
            console.log(gb.concepts[target]);
            fs.writeFileSync(rfname, gb.concepts[target]);
        }
        ...
        if (gb.triple[attribute][gb.SOURCE] == target){ //if source is its own target, its a cycle!
            if (f.is_new_cycle(path, pathSize)){
                ...
                fs.writeFileSync(rfname, gb.concepts[path[0][0]]);
                gb.numcpaths++;
            }
        }
